...
import deletePayment from "src/payments/mutations/deletePayment";

export const Payment = () => {
  const router = useRouter();
  const paymentId = useParam("paymentId", "number");
  const [deletePaymentMutation] = useMutation(deletePayment);
  const [payment] = useQuery(getPayment, { id: paymentId });
  ...
  return (
    <>
      ...
      <div>
        ...
        <Link href={Routes.EditPaymentPage({ paymentId: payment.id })}>
          <a>Edit</a>
        </Link>
        ...
        <button
          ...
          onClick={async () => {
            if (window.confirm("This will be deleted")) {
              await deletePaymentMutation({ id: payment.id });
              await router.push(Routes.PaymentsPage());
            }
          }}
